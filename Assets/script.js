...
const collectEmployees = function() {
  ...
  let addEmployee = true;
    while (addEmployee){
      ...
      let salary;
        do{ salary = parseInt( prompt ("Enter salary amount"));
          if (isNaN(salary)) {alert ("Enter valid salary amount")};
        }  while(isNaN(salary));
        const employee ={
            firstName: firstName,
            lastName: lastName,
...
const displayAverageSalary = function(employeesArray) {
  ...
  for (let i = 0; i < employeesArray.length; i++) {
    if (!isNaN(employeesArray[i].salary)) {
        totalSalarySum += employeesArray[i].salary;
    } 
    
  }
const averageSalary = totalSalarySum / employeesArray.length
